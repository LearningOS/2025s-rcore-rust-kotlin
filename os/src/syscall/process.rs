...
use crate::{
    ...
    mm::{translated_byte_buffer, translated_refmut, translated_str, MapPermission, PageTable, VPNRange, VirtAddr},
    task::{
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let slice = unsafe { core::slice::from_raw_parts(&time as *const TimeVal as *const u8, len) };
    let mut temp = 0;
    for i in buffer {
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    ...
    for vpn in VPNRange::new(start_va.floor(), end_va.ceil()) {
        ...
        if let Some(pte) = pte {
            if pte.is_valid() {
                return -1; // The range is already mapped
            }
        }
    }
    ...
    let task = current_task().unwrap();
    task.map_memory_area(start_va, end_va, flags);
    0
}
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    let task = current_task().unwrap();
    task.unmap_memory_area(start_va, end_va)
}
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        ...
        current_task().unwrap().inner_exclusive_access().children.push(new_task);
        new_pid as isize
    } else {
        -1
    }
